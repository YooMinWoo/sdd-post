...
이 프로젝트는 Spring Boot 기반 게시판 애플리케이션이다. 게시글, 댓글, 작성자, 권한, 검색 같은 기능으로 확장될 수 있으므로 기능별 요구사항과 도메인 규칙을 명확히 분리해서 관리해야 한다.
