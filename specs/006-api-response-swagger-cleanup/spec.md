...
- 이 명세의 최초 구현 당시 실패 응답은 `message`에 영문 에러 코드를 담았으나, 이후 `007-global-exception-policy`와 `008-post-create-auth-korean-errors`에서 `code=<ERROR_CODE>`, `message=<한글 설명>`으로 확장한다.
