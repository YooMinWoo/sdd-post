...
- 실패 응답은 기존 표준 API 응답 정책에 따라 `message`에 대표 에러 코드를 담는다.
...
- 실패 응답의 `message`는 사용자 문구가 아니라 안정적인 에러 코드 문자열을 사용한다.
...
- 실패 응답은 `success=false`, `message=<ERROR_CODE>`, `data=null`, `path`, `timestamp`, `errors`를 포함한다.
- 세부 필드 오류가 필요한 경우 `errors` 배열에 필드명과 필드별 에러 코드를 담는다.
- 대표 에러 코드는 항상 `message`에 유지한다.
