...
- 실패 응답은 `ErrorCode.code()`를 `code` 필드로, `ErrorCode.description()`을 한글 `message` 필드로 변환한다.

## 에러 응답 매핑

| BC | 예외/상황 | 응답 code | 응답 message | HTTP 상태 |
| --- | --- | --- | --- | --- |
...
- `008-post-create-auth-korean-errors`는 실패 응답의 한글 `message`와 영문 `code` 분리 정책을 구체화한다.

## 테스트 전략

- 실제 구현 단계에서는 Web 어댑터 테스트에서 예외별 HTTP 상태, `code=<ERROR_CODE>`, `message=<한글 설명>`을 검증한다.
